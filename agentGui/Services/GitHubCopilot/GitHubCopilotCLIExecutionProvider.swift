...
final class GitHubCopilotCLIExecutionProvider: ConversationExecutionProvider {
    ...
    func send(_ request: ConversationExecutionRequest) async throws {
        ...
        do {
            let remoteBinding = await sessionBridge.binding(for: request.session.sessionId)
    ...
    func cancel(session: Session, modelContext: ModelContext) async {
        ...
        if let remoteSessionID = await sessionBridge.binding(for: session.sessionId)?.remoteSessionID,
    ...
    private func resetRuntime(for localSessionID: String) async {
        ...
        await sessionBridge.removeBinding(for: localSessionID)
