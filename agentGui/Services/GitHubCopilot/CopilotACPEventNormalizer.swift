...
enum CopilotACPUpdate: Equatable, Sendable {
    case session(ACPSessionUpdate)
    case permission(ACPRequestPermissionRequest)
}

enum CopilotNormalizedEvent: Equatable, Sendable {
...
struct CopilotACPEventNormalizer {
    func normalize(update: CopilotACPUpdate) -> [CopilotNormalizedEvent] {
        switch update {
    ...
    private func normalize(sessionUpdate: ACPSessionUpdate) -> [CopilotNormalizedEvent] {
