...
struct FeishuClientLiveTests {
    @Test func startAndSendEmitDebugLogs() async throws {
        ...
        #expect(logs.contains(where: { $0.contains("send text message chat_id=oc_chat_1") }))
        #expect(logs.contains(where: { $0.contains("send text message succeeded message_id=om_sent_1") }))
    }
